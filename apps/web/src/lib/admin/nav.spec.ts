...
describe("adminNavItems (permission-aware navigation)", () => {
  ...
  it("shows Content only when the user holds cms.view (Milestone 8E)", () => {
    const withView = adminNavItems({
      "cms.view": { corporate: true, locationIds: [] },
    });
    expect(withView.map((i) => i.key)).toEqual(["dashboard", "content"]);
    expect(withView.find((i) => i.key === "content")?.href).toBe(
      "/admin/content",
    );

    const manageOnly = adminNavItems({
      "cms.manage": { corporate: true, locationIds: [] },
    });
  ...
  it("shows Administration when the user holds platform.view only (Milestone 5G)", () => {
    const items = adminNavItems({
      "platform.view": { corporate: true, locationIds: [] },
    });
    expect(items.map((i) => i.key)).toEqual(["dashboard", "administration"]);
  });

  it("hides Administration without users.view, roles.view, audit.view or platform.view", () => {
    const caps: AdminCapabilities = {
      "orders.view": { corporate: true, locationIds: [] },
      "catalog.view": { corporate: true, locationIds: [] },
      "locations.view": { corporate: true, locationIds: [] },
